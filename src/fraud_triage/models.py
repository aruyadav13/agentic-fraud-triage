    ...
    def invoke(self, messages):
        ...
        if len(results) < 2:
            return AIMessage(content="", tool_calls=[
                {"name": "account_history", "args": {"transaction_id": case["transaction_id"]},
                 "id": "history"},
                {"name": "velocity", "args": {"transaction_id": case["transaction_id"],
                 "window_hours": 24}, "id": "velocity"},
        ...
        return AIMessage(content="", tool_calls=[{
            "name": "submit_verdict", "id": "verdict", "args": {
...
def huggingface_model(model_id=None):
    ...
    endpoint = HuggingFaceEndpoint(
        ...
        task="text-generation", max_new_tokens=512, do_sample=False,
        timeout=60, huggingfacehub_api_token=token,
    )
