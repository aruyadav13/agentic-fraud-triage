...
def fetch(path: Path, rows: int = 100_000) -> dict:
    ...
    with urllib.request.urlopen(URL, timeout=90) as response:
        ...
        for row in reader:
            records.append({
                ...
                "amount": float(row["amt"]), "is_fraud": int(row["is_fraud"]),
            })
            if len(records) >= rows:
                break
    ...
    manifest = {
        "source": SOURCE, "revision": REVISION, "url": URL, "rows": len(frame),
        "sha256": hashlib.sha256(path.read_bytes()).hexdigest(),
        "kind": "public_simulated_sparkov", "selection": "first N source rows, then time sorted",
        "time_min": str(frame.timestamp.min()), "time_max": str(frame.timestamp.max()),
...
def demo(rows: int = 5000, seed: int = 42) -> pd.DataFrame:
    ...
    return validate(pd.DataFrame({
        ...
        "amount": amount.round(2), "is_fraud": fraud.astype(int),
    }))
